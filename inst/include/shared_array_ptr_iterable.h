...
//			std::cout << "Created shared_array_ptr from *aData. Count = " << *myCount << ", offset = " << myOffset << ", length= " << myLength << ", pointer = " << myData << std::endl;
	...
	// 1. Copy constructor
	shared_array_ptr<T, Alloc>(const shared_array_ptr<T, Alloc>& old_ptr) : myDataSize(old_ptr.myDataSize), myData(old_ptr.myData), myCount(old_ptr.myCount), myOffset(old_ptr.myOffset), myLength(old_ptr.myLength), cleanUp(old_ptr.cleanUp) {
		(*myCount)++;
//			std::cout << "Copied shared_array_ptr. Count = " << *myCount << ", offset = " << myOffset << ", length= " << myLength << ", pointer = " << myData << std::endl;
	...
	/*shared_array_ptr<T, Alloc>(shared_array_ptr<T, Alloc>&& old_ptr) : myDataSize(move(old_ptr.myDataSize)), myData(move(old_ptr.myData)), myCount(move(old_ptr.myCount)), myOffset(move(old_ptr.myOffset)), myLength(move(old_ptr.myLength)), cleanUp(move(old_ptr.cleanUp)) {
		//(*myCount)++;
	...
	shared_array_ptr<T, Alloc>& operator=(const shared_array_ptr<T, Alloc>& rhs) {
		if (this != &rhs) {
			releaseData();
			myData = rhs.myData;
			myCount = rhs.myCount;
...
//			std::cout << "Assigned shared_array_ptr. Count = " << *myCount << ", offset = " << myOffset << ", length= " << myLength << ", pointer = " << myData << std::endl;
	...
	bool operator==(const shared_array_ptr<T, Alloc>& rhs) {
		return this == &rhs;
	}
	...
	shared_array_ptr<T, Alloc> operator+ (const std::ptrdiff_t& anOffset) const {
		...
		return shared_array_ptr<T, Alloc>(*this, myOffset + anOffset, myLength - anOffset);
	}
	...
	shared_array_ptr<T, Alloc> operator- (const std::ptrdiff_t& anEndOffset) const {
		...
		return shared_array_ptr<T, Alloc>(*this, myOffset, myLength - anEndOffset);
	}
	...
	shared_array_ptr<T, Alloc> operator> (const size_t& aLength) const {
		...
		return shared_array_ptr<T, Alloc>(*this, myOffset, aLength);
	}
	// Output operator
	friend std::ostream& operator<<(std::ostream& os, const shared_array_ptr<T, Alloc>& ptr) {
		os << ptr[0];
...
template <class IterT> struct iterator_traits {
